...
export default function SignUpPage() {
  ...
  async function submit(e:FormEvent){e.preventDefault();setMessage('');try{const supabase=createClient();const {error}=await supabase.auth.signUp({email,password,options:{data:{display_name:name}}});if(error)throw error;setMessage('Account created. Check your email if confirmation is enabled, then sign in.');}catch(err){setMessage(err instanceof Error?err.message:'Sign-up failed');}}
