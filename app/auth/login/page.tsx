...
export default function LoginPage() {
  ...
  return <main className="authShell"><form className="authCard" onSubmit={submit}><div className="brand">SPORTS SYNDICATE</div><span className="eyebrow">FANTASY</span><h1>Sign in</h1>{sessionExpired&&<div className="notice">Your session expired in this app. Sign in again to continue.</div>}{confirmation==="verified"&&<div className="notice">Your email has been confirmed. Please sign in to continue.</div>}{confirmation==="failed"&&<div className="notice">This confirmation link is invalid or expired. Try signing in, or request a new confirmation email.</div>}<label>Email<input className="textInput" type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label>Password<input className="textInput" type="password" value={password} onChange={e=>setPassword(e.target.value)} required/></label>{message&&<div className="notice">{message}</div>}<button className="primary" type="submit">Sign In</button><a href="/auth/sign-up">Create an account</a></form></main>;
}
