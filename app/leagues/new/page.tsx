...
export default function NewLeaguePage(){
  const [form,setForm]=useState({organizationName:'',name:'',sport:'NFL',poolType:'SURVIVOR',season:'2026',maxEntries:'1',maxParticipants:'1000',deadlineMode:'GAME_KICKOFF',strictMissedPicks:true});
  ...
  return <main className="shell"><section className="stack">
    ...
    {created?<div className="wideCard leagueForm"><span>League created</span><h2>{created.name}</h2><p>Send this link to your members. It opens the invite code automatically and remembers their place through sign-up.</p><div className="paymentTotal"><span>Invite code</span><strong>{created.inviteCode}</strong></div><div className="inviteShareLink">{typeof window!=='undefined'?`${window.location.origin}/join?code=${created.inviteCode}`:''}</div><a className="primary" href={`/join?code=${created.inviteCode}`}>Preview Join Link</a><a className="secondary" href="/">Open Dashboard</a></div>:<form className="wideCard leagueForm" onSubmit={createLeague}>
