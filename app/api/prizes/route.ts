...
async function manageablePoolIds(supabase:SupabaseClient,userId:string){
 const {data:ownedOrgs}=await supabase.from('organizations').select('id').eq('owner_user_id',userId);
 const {data:staffOrgs}=await supabase.from('organization_members').select('organization_id').eq('user_id',userId).eq('status','ACTIVE').in('role',['OWNER','ADMIN','COMMISSIONER']);
 const orgIds=[...new Set([...(ownedOrgs||[]).map(o=>o.id),...(staffOrgs||[]).map(o=>o.organization_id)])];
 const {data:owned}=orgIds.length?await supabase.from('pools').select('id').in('organization_id',orgIds):{data:[] as {id:string}[]};
 const {data:league}=await supabase.from('league_members').select('pool_id').eq('user_id',userId).eq('status','ACTIVE').in('role',['COMMISSIONER','CO_COMMISSIONER']);
...
export async function GET(){
 try{
  ...
  const {data:prizes,error}=await supabase.from('prizes').select('*,prize_draws(id,winner_entry_id,original_winner_entry_id,winner_snapshot_id,original_winner_snapshot_id,winner_name,original_winner_name,drawn_at,verification_hash,eligible_snapshot,override_reason,overridden_at)').in('pool_id',poolIds).order('draw_at');if(error)throw error;
  const result=[];
  for(const prize of prizes||[]){
   let q=supabase.from('entries').select('id,entry_name,user_id,entry_status,payment_status').eq('pool_id',prize.pool_id).eq('payment_status',prize.eligibility?.payment_status||'PAID');
   ...
   if(prize.eligibility?.one_prize_per_entry){const {data:wins}=await supabase.from('prize_draws').select('winner_entry_id,prizes!inner(pool_id)').eq('prizes.pool_id',prize.pool_id);const won=new Set((wins||[]).map((x:any)=>x.winner_entry_id));eligible=eligible.filter(e=>!won.has(e.id))}
 ...
 }catch(e){const message=e instanceof Error?e.message:'Could not load prizes.';console.error('[api/prizes] GET failed',{message});return NextResponse.json({error:message},{status:message==='UNAUTHENTICATED'?401:500})}
}
export async function POST(req:Request){
 try{
  const {supabase,user}=await requireUser();const body=await req.json();
  ...
  const canManage=await manageablePoolIds(supabase,user.id);if(!canManage.has(poolId))return NextResponse.json({error:'Commissioner access required.'},{status:403});
  ...
  const {data,error}=await supabase.from('prizes').insert({pool_id:poolId,name,description:String(body.description||'').trim().slice(0,300)||null,value_cents:Math.max(0,Math.round(valueDollars*100))||null,draw_at:body.drawAt||null,status:'UPCOMING',eligibility}).select('*').single();
...
export async function PATCH(req:Request){
 try{
  const {supabase,user}=await requireUser();const body=await req.json(),prizeId=String(body.prizeId||'');
  const {data:prize,error}=await supabase.from('prizes').select('id,pool_id,eligibility').eq('id',prizeId).single();if(error||!prize)throw error||new Error('Prize not found.');
  const canManage=await manageablePoolIds(supabase,user.id);if(!canManage.has(prize.pool_id))return NextResponse.json({error:'Commissioner access required.'},{status:403});
