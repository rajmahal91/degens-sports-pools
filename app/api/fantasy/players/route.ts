...
export async function GET(req:Request){
 try{
  ...
  const {data:entry}=await supabase.from('entries').select('id,user_id,pool_id,status').eq('id',entryId).single();
  ...
  return NextResponse.json({players:athletes.map(a=>({...a,used:usedIds.has(a.id),kickoff:kickoff.get(a.team_code)||null})),entryEligible:entry.status==='ACTIVE'});
