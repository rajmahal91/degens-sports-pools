...
export async function POST(request: Request) {
  try {
    ...
    await supabase.from('commissioner_audit_log').insert({ commissioner_id: user.id, action: 'VERIFY_PAYMENT', entity_type: 'payment', entity_id: paymentId, before_state: before, after_state: data });
