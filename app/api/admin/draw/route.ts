...
export async function POST(req:Request){
 try{
  ...
  const {error:insertError}=await s.from('prize_draws').insert({id:drawId,prize_id:prize.id,winner_entry_id:winner.id,original_winner_entry_id:winner.id,eligible_snapshot:snapshot,random_value:randomValue,verification_hash:verificationHash,drawn_by:user.id,drawn_at:new Date().toISOString()});
...
export async function PATCH(req:Request){
 try{
  ...
  const before={winner_entry_id:draw.winner_entry_id,override_reason:draw.override_reason};
  const patch={winner_entry_id:winnerEntryId,override_reason:String(reason).trim().slice(0,500),overridden_at:new Date().toISOString(),overridden_by:user.id};
