...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('payments').insert({ user_id: user.id, pool_id: poolId, entry_id: entryId, amount_cents: pool.entry_fee_cents, currency: pool.currency, method, status: 'PENDING', payer_reference: reference || null }).select().single();
