...
export async function GET(){
  try{
    ...
    return NextResponse.json({configured:pushIsConfigured(),enabled:Boolean(preferences?.push_enabled&&subscriptions?.length),subscriptionCount:subscriptions?.length||0,preferences:preferences||{push_enabled:true,pick_reminders:true,results_updates:true}});
