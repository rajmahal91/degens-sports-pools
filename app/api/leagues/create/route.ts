...
export async function POST(request:Request){
  try{
    ...
    if(!organizationId&&!organizationName){
      const {data:ownedOrg}=await supabase.from('organizations').select('id').eq('owner_user_id',user.id).order('created_at').limit(1).maybeSingle();
      organizationId=ownedOrg?.id||'';
    }
    if(!organizationId){
      const orgName=organizationName||`${name} Organization`;
      const slug=`${slugify(orgName)}-${randomBytes(3).toString('hex')}`;
      const {data:org,error:orgError}=await supabase.from('organizations').insert({name:orgName,slug,owner_user_id:user.id}).select('id,name').single();
    ...
    const {error:leagueMemberError}=await supabase.from('league_members').insert({pool_id:pool.id,user_id:user.id,role:'COMMISSIONER',status:'ACTIVE'});
    ...
    const {error:roundError}=await supabase.from('rounds').insert(rounds);
    ...
    const {error:inviteError}=await supabase.from('league_invitations').insert({pool_id:pool.id,code_hash:codeHash,code_hint:inviteCode.slice(-4),created_by:user.id});
