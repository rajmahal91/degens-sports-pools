...
export async function GET(request:Request){
  try{
    ...
    const sync=await refreshWeek(season,week);
    const [{data:games,error:gamesError},{data:entries,error:entriesError}]=await Promise.all([
      supabase.from('games').select('*').eq('sport','NFL').eq('season',season).eq('week',week).order('kickoff_at'),
    ...
    const {data:picks,error:picksError}=entryIds.length
      ?await supabase.from('survivor_picks').select('entry_id,week,result').in('entry_id',entryIds).eq('week',week)
      :{data:[],error:null};
