-- The platform does not collect, process, verify, or use payments.
-- Legacy financial columns remain temporarily for backward-compatible reads,
-- but all pool fees are normalized to zero and eligibility ignores payment state.
update public.pools
...
begin
  ...
  if v_pool_type = 'SURVIVOR' then
    return query
      select e.id, e.entry_name, e.entry_status::text,
...
revoke all on function public.get_leaderboard_rows(uuid) from anon;
